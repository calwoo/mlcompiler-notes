...
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    ...
    return std::count_if(
        curr_set.begin(), 
        curr_set.end(),
        [&target_set](const string& link) { return (target_set.find(link) != target_set.end()); }
    );
}
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    auto cmp_fn = [&w, &target_set](const vector<string>& left, const vector<string>& right) {
        // replace all of these lines.
        const string& left_last_page = left.back();
        const string& right_last_page = right.back();
        unordered_set<string> left_common_pages = w.getLinkSet(left_last_page);
        unordered_set<string> right_common_pages = w.getLinkSet(right_last_page);
        int num_left_common_links = numCommonLinks(left_common_pages, target_set);
        int num_right_common_links = numCommonLinks(right_common_pages, target_set);
        return (num_left_common_links < num_right_common_links);
    };
    // END STUDENT CODE HERE
    ///////////////////////////////////////////////////////////////////////////////////////////////////
