...
prop_optimizer_idempotent e = simplifier (simplifier e) == simplifier e
...
prop_optimizer_optimizes e = not (findPlusZero (peephole simplifier e))
...
peephole opt (Lit n) = opt (Lit n)
peephole opt (Op op e1 e2) = 
    if e1 == s1 && e2 == s2 
        then opt (Op op s1 s2) 
        else peephole opt (Op op s1 s2)
    where
        s1 = peephole opt e1
        s2 = peephole opt e2
...
step (IPush n) st = Just (n:st)
step (IOp b) (x:y:is) = Just ((interpBinOp b y x):is)
step _ _ = Nothing
...
run [] st = Just st
run (i:is) st = step i st >>= \s -> run is s
