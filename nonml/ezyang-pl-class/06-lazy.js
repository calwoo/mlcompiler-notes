...
function zipWith(f, thunk_xs, thunk_ys) {
    ...
    return delay(function() {
        var x0 = force(thunk_xs);
        var y0 = force(thunk_ys);
        return {
            head: f(x0.head, y0.head),
            tail: zipWith(f, x0.tail, y0.tail),
        }
    })
}
...
console.log(take(5, tail(enumFrom(3))));

function cons(x, thunk_ys) {
    return delay(function() {
        return {
            head: x,
            tail: thunk_ys,
...
console.log(take(5, cons(0, enumFrom(2))));
