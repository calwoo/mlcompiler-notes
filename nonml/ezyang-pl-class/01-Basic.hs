...
padZeros fullLen digits = 
  if length digits < fullLen then 0 : padZeros (fullLen - 1) digits else digits
...
evenDigitsExcludingLast [] = []
evenDigitsExcludingLast [x] = []
evenDigitsExcludingLast [x, y] = []
evenDigitsExcludingLast (x:y:xs) = y : evenDigitsExcludingLast xs
...
checkDigit digits =
  if rem == 0 then 0 else 10 - rem
    where
      oddSum = sum (oddDigits digits)
      evenSum = sum (evenDigitsExcludingLast digits)
      rem = (oddSum * 3 + evenSum) `mod` 10
...
checkUPC n = checkDigit upcDigits == last upcDigits
  where
    upcDigits = padZeros 12 (toDigits n)
